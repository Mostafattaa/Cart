...
const Items = ({products, reset, deleteAll, increment, decrement, deleteItem, theme, switchTheme}) => {
  ...
  return (
    <div className={`flex flex-col justify-start items-center gap-10 p-5 pt-20 `}>
        ...
        {total != 0 && <h3 className={`text-2xl bg-orange-400 shadow-lg p-2.5 rounded-lg ${!theme ? 'text-[#000000]' : 'text-[#ffffff]'}`}>Total: {total} EGP</h3>}
        ...
        {total == 0 ? <h3 className={`${!theme ? 'text-[#000000]' : 'text-[#ffffff]'} text-2xl`}>Your card is empty!</h3> : <h3 className={`text-2xl bg-orange-400 shadow-lg p-2.5 rounded-lg ${!theme ? 'text-[#000000]' : 'text-[#ffffff]'}`}>Total: {total} EGP</h3>}
    </div>
  )
}
